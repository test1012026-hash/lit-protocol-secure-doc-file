...
export default function Inbox({ auth }) {
  ...
  useEffect(() => {
    api.inbox(auth.token).then(({ data }) => setFiles(data)).catch(() => {});
  }, [auth.token]);
  ...
  const handleDecrypt = async (fileId) => {
    try {
      ...
      chrome.downloads.download({ url, filename: file.filename || file.subject });
  ...
  return (
    <div>
      ...
      {files.map((f) => (
        <div key={f._id} style={{ marginBottom: 12, paddingBottom: 8, borderBottom: '1px solid #eee' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <strong style={{ fontSize: 13 }}>{f.subject}</strong>
            <button onClick={() => handleDecrypt(f._id)}>Decrypt</button>
          </div>
          {f.message && <p style={{ fontSize: 12, color: '#555', margin: '0 0 4px' }}>{f.message}</p>}
          {f.filename && f.filename !== f.subject && (
            <p style={{ fontSize: 11, color: '#888', margin: 0 }}>File: {f.filename}</p>
          )}
        </div>
      ))}
      {status && <p>{status}</p>}
