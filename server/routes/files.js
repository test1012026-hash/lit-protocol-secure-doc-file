...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    const { recipientEmail, ciphertext, dataToEncryptHash, litActionCode, subject, message, filename } = req.body;
    if (!recipientEmail || !ciphertext || !dataToEncryptHash || !litActionCode) {
    ...
    if (!recipient) {
      recipient = new User({ email, claimed: false, uuid: crypto.randomUUID() });
    ...
    res.json({ fileId: file._id, recipientUuid: recipient.uuid, recipientClaimed: recipient.claimed });
  } catch (err) {
